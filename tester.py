...
def test_grapple_commonality(c1, c2, spec):
    """
    ...
    """
    c1_ee1x, c1_ee1y = c1.points[0]
    c1_ee2x, c1_ee2y = c1.points[-1]
    c2_ee1x, c2_ee1y = c2.points[0]
    c2_ee2x, c2_ee2y = c2.points[-1]
    if point_is_close(c1_ee1x, c1_ee1y, c2_ee1x, c2_ee1y, spec.TOLERANCE) or \
            point_is_close(c1_ee2x, c1_ee2y, c2_ee2x, c2_ee2y, spec.TOLERANCE):
        return True
    else:
        return False
...
def main(arglist):
    ...
    for i in range(len(robot_configs)):
        ...
        if i + 1 < len(robot_configs) and not test_grapple_commonality(robot_configs[i], robot_configs[i + 1], spec):
            violations += 1
            if violations <= 10:
                print("!!! Robot jumped between grapple points between step number " + str(i) + " and " +
                      str(i + 1) + " !!!")
            major_violations += 1
